import { $n as Vector3, At as Matrix3, Qn as Vector2 } from "./three.module-DBWZZ85L.js";
...
var noise = /* @__PURE__ */ Object.freeze({
	__proto__: null,
	seed,
	simplex2: function simplex2(xin, yin) {
		var n0, n1, n2;
		var s = (xin + yin) * F2;
	...
	simplex3: function simplex3(xin, yin, zin) {
		var n0, n1, n2, n3;
	...
	_createClass(FlashGen, [
		{
			...
			value: function scheduleNextBurst(currentTime) {
				this.nextBurstTime = currentTime + (Math.random() * (this.maxInterval - this.minInterval) + this.minInterval) / 1e3;
				this.flashesDone = 0;
		...
		},
		{
			...
			value: function update(currentTime, delta) {
				if (currentTime > this.nextBurstTime && this.currentCount === 0) this.currentCount = Math.floor(Math.random() * this.count) + 1;
				if (this.flashesDone < this.currentCount && currentTime > this.nextBurstTime) {
					if (!this.isFlashing) {
						this.isFlashing = true;
						this.flashIntensity = 1;
						this.nextFlashEndTime = currentTime + (Math.random() * (this.maxDuration - this.minDuration) + this.minDuration) / 1e3;
...
function inRect(buffer, rect) {
	...
	for (var i = 0; i < buffer.length; i += 2) {
		buffer[i] = (rng.value() - .5) * sideX + center[0];
		buffer[i + 1] = (rng.value() - .5) * sideY + center[1];
	}
	return buffer;
}
...
function inBox(buffer, box) {
	...
	for (var i = 0; i < buffer.length; i += 3) {
		buffer[i] = (rng.value() - .5) * sideX + center[0];
		buffer[i + 1] = (rng.value() - .5) * sideY + center[1];
		buffer[i + 2] = (rng.value() - .5) * sideZ + center[2];
	}
	return buffer;
}
var defaultBox = {
...
//#endregion
export { _setPrototypeOf as _, inRect as a, onBox as c, onSphere as d, _classCallCheck as f, _isNativeReflectConstruct as g, _toConsumableArray as h, inCircle as i, onCircle as l, misc as m, Generator as n, inSphere as o, deltaAngle as p, inBox as r, noise as s, FlashGen as t, onRect as u };

//# sourceMappingURL=index-0332b2ed.esm-DFn3N9-B.js.map